...
void Memory::requestMemoryOperation(int PID, int memoryAddress) {
   ...
   int pageNumber_ = ramMemory_ / memoryAddress;
...
void Memory::updateFrameTable(int PID, int pageNumber) {
   ...
   if (frameTable_.size() == numOfFrames_) {
      ...
      frameTable_[currFrameTableNum_]->pageNumber_ = pageNumber;
      frameTable_[currFrameTableNum_]->PID_ = PID;
      frameTable_[currFrameTableNum_]->timeStamp_ = timeStamp_+1;
   }
}
